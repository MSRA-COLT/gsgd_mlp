...
class gSGD():
    ...
    def cal_ec_layer_type1(self):
        ...
        params_idx_dict={}
        for idx, (ii,jj) in enumerate(named_params):
            params_idx_dict[ii]=idx
        named_params = self.models.named_parameters()
        layers =unique(['.'.join(x[0].split('.')[:3])  for x in named_params])[1:]
        layers_idx_dict={}
        for idx, ii in enumerate(layers):
            layers_idx_dict[ii]=idx

        ec_layer = [[] for _ in layers]
        for (ii, params_idx) in params_idx_dict.items():

            if '.fc' in ii and '.weight' in ii:
                print(ii, params_idx, layers_idx_dict.keys())
                layer_idx = layers_idx_dict['.'.join(ii.split('.')[:3])]
                ec_layer[layer_idx].append(params_idx)
            else:
                continue
        return(ec_layer)
    ...
    def cal_mask(self, model):
        ...
        for ec_layer in self.ec_layer:
            layer_mask = []
            layer_s_idx =[]
            layer_s_h = []
            # layer_s_mask_idx =[]
            num_sub_layer = len(ec_layer)

            block_s_mask_idx = None

            tmp = [0,0]
            for ii in range(num_sub_layer-1):
                h = self.params[ec_layer[ii]].shape[0]
                if h > tmp[1]:
                    tmp = [ii , h]
            ...
            for sub_layer_idx, sub_layer in enumerate(ec_layer):
                outcome, income  = self.params[sub_layer].shape

                if sub_layer_idx == 0:
                    loc = 'f'
                elif sub_layer_idx == num_sub_layer - 1:
                    loc = 'l'
                else:
                    loc = 'm'
                ...
                if loc == 'f':
                    ...
                    if outcome > income:
                        ...
                        mask_tmp = self.generate_eye(outcome, income, loc).to(self.args.device)
                        # print(outcome, income, self.expand)
                        self.shrink_layer_mask(mask_tmp)

                layer_mask.append(mask_tmp)
                this_mask_red = (mask[-1] == 1).to(torch.float32) + (mask[-1] == 3).to(torch.float32) + (
                            mask[-1] == 2).to(torch.float32)
                self.params[sub_layer].data = self.params[sub_layer].data * (1 - this_mask_red) + this_mask_red
            ...
            # s_mask_idx.append(layer_s_mask_idx)
        self.s_h = s_h
        self.s_idx = s_idx
        return (mask, )
